...
describe("Background value parser", function () {
    describe("extractCssUrl", function () {
        ...
        it("should handle double quotes", function () {
            var url = backgroundValueParser.extractCssUrl(
                'url("path/file.png")'
            );
            expect(url).toEqual("path/file.png");
        });
        ...
        it("should handle a data URI with closing bracket when quoted with double quotes", function () {
            var url = backgroundValueParser.extractCssUrl(
                'url("data:image/svg+xml,%3Csvg%20%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Crect%20height%3D%2210%22%20width%3D%2210%22%20style%3D%22transform%3A%20rotate(45deg)%3B%20fill%3A%20%2300f%22%2F%3E%3C%2Fsvg%3E")'
            );
            expect(url).toEqual(
                "data:image/svg+xml,%3Csvg%20%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Crect%20height%3D%2210%22%20width%3D%2210%22%20style%3D%22transform%3A%20rotate(45deg)%3B%20fill%3A%20%2300f%22%2F%3E%3C%2Fsvg%3E"
            );
        });
    ...
    describe("parse", function () {
        ...
        it("should deal with a url mixed with a linear-gradient", function () {
            var values = backgroundValueParser.parse(
                "url(path/file.png), linear-gradient(to bottom right, red, rgba(255,0,0,0))"
            );
        ...
        it("should deal with a url mixed with a linear-gradient", function () {
            var values = backgroundValueParser.parse(
                "linear-gradient(to bottom right, red, rgba(255,0,0,0)), url(path/file.png)"
            );
    ...
    describe("back and forth conversion", function () {
        it("should handle a url", function () {
            var cssValue = backgroundValueParser.serialize(
                backgroundValueParser.parse("url(path/file.png)")
            );
